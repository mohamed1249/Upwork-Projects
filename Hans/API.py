...
data = {
    'amphia' : {
        'model' : load_index_from_storage(StorageContext.from_defaults(persist_dir=r'/home/mohameedmtmn/amphia/amphia/LLM/storage')).as_query_engine(),
...
# Reset history keys thread function
def reset_history_keys():
    global data
    threading.Timer(300.0, reset_history_keys).start()  # 300 seconds = 5 minutes

    for key in data:
        data[key]['history'] = ''

# Start the reset history keys thread
reset_history_keys()
...
def amphia_chat():
    try:
        ...
        if data['amphia']['history']:

            try:
                user_input = f"""Based on this chat history that you and I have been discussing:
                                '{data['amphia']['history'][int(-4096*4/2):]}'
        ...
        #                            '''
        #         bot_response = bot_response.strip('\n') + ' ' + data["amphia"]['model'].query(complete_command).response.strip('\n')
...
def miljuschka_chat():
    try:
        ...
        if data['miljuschka']['history']:
            
            try:
                user_input = f"""Based on this chat history that you and I have been discussing:
                                '{data['miljuschka']['history'][int(-4096*4/2):]}'
                                Answer this question:
                                {question}
                                Providing a detailed response based on your existing knowledge and understanding,
                                Don't generate fictional or made-up information, Stick to factual and relevant details from the provided knowledge,
...
def brabantia_chat():
    try:
        ...
        if data['brabantia']['history']:
            
            try:
                user_input = f"""Based on this chat history that you and I have been discussing:
                                '{data['brabantia']['history'][int(-4096*4/2):]}'
                                Answer this question:
                                {question}
                                Providing a detailed response based on your existing knowledge and understanding,
                                Don't generate fictional or made-up information, Stick to factual and relevant details from the provided knowledge,
